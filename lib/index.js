...
function QCypher() {
   ...
   * @returns {*}
   */
  function queryStatementBuilder(template, params) {
    var temp = us.template(template);
    return temp(params);
  }
  ...
  /**
   * getSimpleData
   * Simple data is defined as a single return value.  A single object qualifies
   * @param result
   * @returns {*}
   */
  function getSimpleData(result) {
    var ret = null;
    if (!result || !result.data || result.data.length === 0) {
      return ret;
    }
    if (result.columns.length === 1) {
      ret = (typeof result.data[0][0].data === 'object') ? result.data[0][0].data : result.data[0][0];
    }
    return ret;
  }
  ...
  /**
   * transSingleExecute
   * Execute a single transaction and handle commit or rollback, return promise
   * @param transobj - transaction object
   * @param statements - transactions statements
   * @returns {promise|*|Q.promise}
   */
  function transSingleExecute(transobj, statements) {
    var deferred = q.defer();
    transExecute(transobj, statements)
      .then(function(result) {
        if (result.errors.length) {
          transRollback(transobj);
          deferred.reject(result.errors);
        } else {
          transCommit(transobj)
            .then(function(result) {
              deferred.resolve(result);
            }, function(err) {
              transRollback(transobj);
  ...
  return {
    ...
    transSingleExecute: transSingleExecute,
